...
class FacultyTable {
    ...
    async loadData() {
        try {
            ...
            if (!dataset.updatedAt || !Array.isArray(dataset.faculty)) {
                throw new Error('Invalid faculty dataset format');
            }

            this.facultyData = dataset.faculty;
            this.filteredData = [...this.facultyData];
            this.updateDataTimestamp(dataset.updatedAt);
        } catch (error) {
    ...
    updateDataTimestamp(updatedAt) {
        const updatedElement = document.getElementById('dataUpdated');
        const updatedDate = new Date(updatedAt);

        if (!updatedElement || Number.isNaN(updatedDate.getTime())) {
            throw new Error('Invalid dataset update timestamp');
        }
