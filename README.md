...
The system processes faculty profiles, research classifications,
and committee assignments to create an interactive web-based table
for exploring all students currently or formerly mentored by
faculty at UAB through graduate committee memberships.
...
The UAB Faculty Committee Memberships project addresses the need
for a comprehensive view of faculty involvement in graduate
education through committee memberships. While this data exists
on individual UAB Scholars pages, this system provides an easy
way to view all students currently or formerly mentored by faculty
at UAB. By combining data from UAB Scholars profiles and graduate
committee records, this system provides insights into:
...
The project processes structured data from multiple sources to
create a searchable, filterable interface that helps researchers,
administrators, and students understand the mentoring landscape
at UAB through graduate committee memberships.
...
1. Fetches all faculty profiles from UAB Scholars
   → `data/uab_scholars_profiles.jsonl`
2. Submits a SLURM array job (128 chunks) that, for each
   faculty member:
   - Fetches committee memberships → `data/committees_by_id/`
   - If the faculty has students, fetches email and
     publication/grant keywords → `data/faculty_data/`

#### Step 2: Wait for completion

- 128 parallel array jobs process faculty in chunks
- Failed faculty are automatically retried within each task
...
- **Committee Memberships**: View all students mentored through
  graduate committees
- **Responsive Design**: Mobile-friendly interface
- **Real-time Statistics**: Dynamic updates during filtering
- **Hidden Keywords**: Publication and grant keywords for enhanced
  search
...
├── src/                               <- All scripts
│   ├── fetch_uab_data.py             <- Unified data collection
│   ├── create_faculty_student_data.py <- Data processing
│   ├── create_html_table.py           <- HTML table generation
│   ├── update_docs.py                 <- Documentation updates
│   ├── merge_committee_chunks.py      <- Merge committee chunks
│   ├── inspect_api_data.py            <- API data inspection
│   ├── run_pipeline.sh               <- Pipeline entry point
│   └── submit_pipeline.sh            <- SLURM array job
...
│   └── processed/
│       └── faculty_students.json      <- Combined data for table
...
- Shaurita Hutchins
  [email](mailto:sdhutchins@uab.edu) |
  PhD Candidate |
